...
object PrinterManager {
    ...
    fun init(context: Context, onReady: () -> Unit = {}) {
        ...
        try {
            ...
            InnerPrinterManager.getInstance().bindService(context.applicationContext,
                object : InnerPrinterCallback() {
                    override fun onConnected(service: SunmiPrinterService?) {
                        printerService = service; connected = (service != null)
                        NjmApp.sunmiPrinter = service; NjmApp.printerConnected = connected
                        Log.d(TAG, if (connected) "Connected" else "Not Sunmi")
                        onReady()
                    }
                    override fun onDisconnected() {
                        printerService = null; connected = false; NjmApp.printerConnected = false
                        onReady()
                    }
                })
        } catch (e: InnerPrinterException) { connected = false; onReady() }
        catch (e: Exception) { connected = false; onReady() }
    }
    ...
    fun printWashReceipt(
        ...
        washDate: String = "", washTime: String = ""
    ) {
        val svc = printerService ?: run { Log.w(TAG, "Not connected"); return }
        try {
            svc.printerInit(null)
            svc.setAlignment(1, null); svc.setFontSize(30f, null)
            ...
            svc.printText("شكراً لزيارتكم | meshari.tech\n\n\n", null)
            svc.cutPaper(null)
        } catch (e: Exception) { Log.e(TAG, "printWashReceipt: ${e.message}") }
    }

    fun printDailyReport(
        context: Context, washes: List<WashRecord>, date: String,
        ...
        try {
            ...
            svc.printText("================================\n", null)
            svc.setAlignment(1, null); svc.printText("meshari.tech\n\n\n", null)
            svc.cutPaper(null)
        } catch (e: Exception) { Log.e(TAG, "printDailyReport: ${e.message}") }
    }

    fun printMonthlyReport(
        context: Context, washes: List<WashRecord>, month: String,
        totalOverride: Double = -1.0, paidOverride: Double = -1.0, unpaidOverride: Double = -1.0
    ) {
        val svc = printerService ?: return
        val total = if (totalOverride >= 0) totalOverride else washes.sumOf { it.cost ?: 0.0 }
